...
class ExperienceReplay:
    ...
    def append(self, observation, action, reward, done, next_observation=None):
        ...
        if self.store_next:
            if self.symbolic_env:
                self.next_observations[self.idx] = next_observation.numpy()
            else:
    ...
    def _retrieve_batch(self, idxs, n, L):
        ...
        if self.store_next:
            next_observations = torch.as_tensor(self.next_observations[vec_idxs].astype(np.float32))
            if not self.symbolic_env:
                preprocess_observation_(next_observations, self.bit_depth)  # Undo discretisation for visual observations
            return (
                ...
                next_observations.reshape(L, n, *observations.shape[1:]),
            )
