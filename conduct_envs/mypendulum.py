...
_COSINE_UPPER_BOUND = np.cos(np.deg2rad(0))
...
def swingup_angle(time_limit=_DEFAULT_TIME_LIMIT, random=None,
                  environment_kwargs=None, angle_lower_bound=_COSINE_BOUND,
                  angle_upper_bound=_COSINE_UPPER_BOUND):
    """Returns pendulum swingup task ."""
    physics = Physics.from_xml_string(*get_model_and_assets())
    task = SwingUp(random=random, angle_lower_bound=angle_lower_bound,
...
class SwingUp(base.Task):
    ...
    def __init__(self, random=None, angle_lower_bound=_COSINE_BOUND,
                 angle_upper_bound=_COSINE_UPPER_BOUND):
        ...
        print('In this environment, the angle lower bound is {}, '
              'the angle upper bound is {}'.format(self.angle_lower_bound,
                                                   self.angle_upper_bound))
    ...
    def get_reward(self, physics):
        # print('angle', physics.pole_vertical())
        # print('rewards', rewards.tolerance(physics.pole_vertical(), (_COSINE_BOUND, 1)))
        return rewards.tolerance(physics.pole_vertical(), (self.angle_lower_bound,
                                                           self.angle_upper_bound))
