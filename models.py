...
# reward transition model: q(s_{t+1}|h_{t+1}, o_{t+1}) -> q(s_{t+1}|h_{t+1}, o_{t+1}, r_t)
class TransitionModel(jit.ScriptModule):
    ...
    def __init__(
        ...
        if self.encoder_r:
            self.fc_embed_belief_posterior = nn.Linear(belief_size + embedding_size + 1,
                                                       hidden_size)
        else:
            self.fc_embed_belief_posterior = nn.Linear(belief_size + embedding_size,
                                                       hidden_size)
        # print(belief_size + embedding_size)
    ...
    # Diagram of expected inputs and outputs for T = 5
    # (-x- signifying beginning of output belief/state that gets sliced off):
    # t :  0  1  2  3  4  5
    # inputs:
    # o :    -X--X--X--X--X-
    # a : -X--X--X--X--X-
    ...
    # r : -X--X--X--X--X-
    # outputs:
    # b : -x--X--X--X--X--X-  do not return the first result!!
    # s : -x--X--X--X--X--X-
    # batch size: default 50
    # chuck size: default is 50, we set it as 64 to see the shape
    # print(prev_state.shape) # [50, 200]
    # print(actions.shape) # [63, 50, 6]
    # print(prev_belief.shape) # [50, 30]
    # print(observations.shape) # [63, 50, 1024]
    # print(nonterminals.shape) # [63, 50, 1]
    @jit.script_method
    def forward(
            self,
        ...
        Output: beliefs, prior_states, prior_means,
                torch.Size([63, 50, 200]) torch.Size([63, 50, 30]) torch.Size([63, 50, 30])
                prior_std_devs, posterior_states,
                torch.Size([63, 50, 30]) torch.Size([63, 50, 30])
                posterior_means, posterior_std_devs
                torch.Size([63, 50, 30]) torch.Size([63, 50, 30])
        '''
        # Create lists for hidden states (cannot use single tensor as buffer
        # because autograd won't work with inplace writes)
        T = actions.size(0) + 1  # 49+1=50: chuck_size
        ...
        for t in range(T - 1):
            ...
            # Compute belief (deterministic hidden state) i.e., h_t in the PlaNet
            # h_{t+1} = f(s_t, a_t, h_t) Eq.(4) of RSSM
            hidden = self.act_fn(self.fc_embed_state_action(torch.cat([_state, actions[t]], dim=1)))
            beliefs[t + 1] = self.rnn(hidden, beliefs[t])

            # Compute state prior by applying transition dynamics
            # p(s_{t+1}|h_{t+1}), Gaussian distribution
            # mean, std = prior_means[t + 1], prior_std_devs[t + 1]
            hidden = self.act_fn(self.fc_embed_belief_prior(beliefs[t + 1]))
            prior_means[t + 1], _prior_std_dev = \
                torch.chunk(self.fc_state_prior(hidden), 2, dim=1)
            prior_std_devs[t + 1] = F.softplus(_prior_std_dev) + self.min_std_dev
            prior_states[t + 1] = prior_means[t + 1] + \
                                  prior_std_devs[t + 1] * \
                                  torch.randn_like(prior_means[t + 1])
            # q(s_{t+1}|h_{t+1}, o_{t+1}, r_t)
            if observations is not None:
                # Compute state posterior by applying transition dynamics
                # and using current observation
                t_ = t - 1  # Use t_ to deal with different time indexing for observations

                # the only difference #
                # print(beliefs[t+1].shape) # [50, 200]
                # print(observations[t_+1].shape) # [50, 1024]
                # print(rewards[t].shape) # [50, 5]
                if self.encoder_r:
                    _reward = (
                        rewards[t] if nonterminals is None else rewards[t] * nonterminals[t]
                    )  # Mask if previous transition was terminal
                    hidden = self.act_fn(
                        self.fc_embed_belief_posterior(torch.cat([beliefs[t + 1],
                                                                  observations[t_ + 1],
                                                                  _reward], dim=1))
                    )
                else:
                ...
                posterior_means[t + 1], _posterior_std_dev = \
                    torch.chunk(self.fc_state_posterior(hidden), 2, dim=1)
...
class RewardModel(jit.ScriptModule):
    ...
    @jit.script_method
    def forward(self, belief, state):
        # r_t = p(r_t | h_t, s_t) Eq.(4) in RSSM
...
class TaskModel(jit.ScriptModule):
    def __init__(self, belief_size, state_size, hidden_size, activation_function='relu',
                 task_number=1):
        # [--belief-size: 200, --hidden-size: 200, --state-size: 30]
        super().__init__()
        ...
        self.fc1 = nn.Linear(belief_size + state_size, hidden_size)
        self.fc2 = nn.Linear(hidden_size, hidden_size)
        self.fc3 = nn.Linear(hidden_size, task_number)
        self.modules = [self.fc1, self.fc2, self.fc3]

    @jit.script_method
    def forward(self, belief, state):
        # p(D | h_t, s_t) Eq.(4) in RSSM
        x = torch.cat([belief, state], dim=1)
        hidden = self.act_fn(self.fc1(x))
        hidden = self.act_fn(self.fc2(hidden))
        hidden = self.fc3(hidden)
        # task_log_softmax = F.log_softmax(hidden, dim=1)
        # return task_log_softmax
        return hidden
