...
def imagine_ahead(prev_state, prev_belief, policy, transition_model,
                  planning_horizon=12, task_model=None):
    '''
    ...
    for t in range(T - 1):
        ...
        if task_model is not None:
            task_contexts = task_model(beliefs[t], _state)
            # actor_beliefs = torch.cat([beliefs[t], task_contexts], dim=-1)
            action_state = torch.cat([_state, task_contexts], dim=-1)
            actions = policy.get_action(beliefs[t].detach(), action_state.detach())
        else:
